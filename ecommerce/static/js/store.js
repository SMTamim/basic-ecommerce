...
function purchased() {
    var totalPrice = parseFloat(document.getElementsByClassName('cart-total-price')[0].innerText.replace('৳', ''));
    if (totalPrice <= 0) {
...
function updateCartTotal() {
    ...
    for (var i = 0; i < cartRows.length; i++) {
        var price = parseFloat(cartRows[i].getElementsByClassName('cart-price')[0].innerText.replace('৳', ''));
